defmodule Runcom do
  ...
  @type t :: %__MODULE__{
          ...
          status: status(),
          sink: term() | nil,
          secret_store: Runcom.SecretStore.Memory.t() | nil
        }
  ...
  """
  @spec graft(t(), String.t(), t(), keyword()) :: t()
  ...
  the runbook execution. The Orchestrator owns a process-local SecretStore
  ...
  defp execute_step(
    ...
    case result do
      {:ok, %Runcom.Step.Result{status: :error} = res} ->
        ...
        :telemetry.execute(
          [:runcom, :step, :stop],
          %{duration: duration},
          Map.merge(meta, %{status: :error, result: res})
        )
      ...
      {:ok, %Runcom.Step.Result{} = res} ->
        ...
        :telemetry.execute(
          [:runcom, :step, :stop],
          %{duration: duration},
          Map.merge(meta, %{status: :ok, result: res})
        )
      ...
      {:error, reason} ->
        error_result =
          ...
          |> add_timing(started_at, completed_at, duration_ms, step_order)

        updated_step = StepNode.put_result(updated_step, error_result)
        ...
        rc = %{
          ...
          | steps: Map.put(rc.steps, name, updated_step),
            step_status: Map.put(rc.step_status, name, :error),
            errors: Map.put(rc.errors, name, reason)
        }
