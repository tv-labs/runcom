defmodule Runcom.Formatter.Markdown do
  ...
  defp format_header(rc) do
    ...
    lines = [
      ...
      "",
      "**Status:** #{status_icon} #{format_status(rc.status)}"
    ]

    lines = if duration, do: lines ++ ["**Duration:** #{duration}ms"], else: lines
    ...
    error =
      if status == :error do
        ...
        error_text =
          cond do
            error_reason ->
              Redactor.redact(inspect(error_reason), secrets)
  ...
  defp format_variables(%Runcom{assigns: assigns}, _secrets) when map_size(assigns) == 0, do: nil
    ...
    **Module:** #{inspect(step.module)}
    ...
    header =
      if result && result.duration_ms do
        header <> "**Duration:** #{result.duration_ms}ms\n"
      else
        header
      end
  ...
  defp format_step_output(rc, step, result, secrets) do
    ...
    error_section =
      if result && result.error && result.error != "" do
        error_text = Redactor.redact(inspect(result.error), secrets)
