defmodule Runcom.StepNodeTest do
  ...
  describe "new/4 with sources" do
    test "stores sources map alongside opts" do
      sources = %{cmd: ~S|&("echo #{&1.assigns.msg}")|}
      node = StepNode.new("greet", Runcom.Steps.Command, %{cmd: "echo hi"}, sources)

      assert node.sources == sources
      assert node.opts == %{cmd: "echo hi"}
    end

    test "defaults sources to empty map" do
      node = StepNode.new("greet", Runcom.Steps.Command, %{cmd: "echo hi"})
      assert node.sources == %{}
    end
  end
