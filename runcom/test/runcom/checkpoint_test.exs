defmodule Runcom.CheckpointTest do
  ...
  describe "source rebuild on read" do
    test "rebuilds from source module when present", %{checkpoint_opts: opts} do
      rc = Runcom.Runbook.build(Runcom.Test.Runbook, %{id: "src"})
      rc = %{rc | status: :halted, step_status: %{"check" => :ok}}

      {mod, params, bytecodes} = rc.source
      assert mod == Runcom.Test.Runbook
      assert params == %{id: "src"}
      assert is_list(bytecodes)

      :ok = Checkpoint.write(rc, opts)
      {:ok, restored} = Checkpoint.read("test-src", opts)

      assert match?({Runcom.Test.Runbook, %{id: "src"}, _}, restored.source)
      assert restored.status == :halted
      assert restored.step_status == %{"check" => :ok}
    end

    test "falls back to raw checkpoint when no source", %{checkpoint_opts: opts} do
      rc = Runcom.new("no-source")
      rc = %{rc | status: :halted}

      :ok = Checkpoint.write(rc, opts)
      {:ok, restored} = Checkpoint.read("no-source", opts)

      assert restored.id == "no-source"
      assert restored.status == :halted
      assert is_nil(restored.source)
    end
  end
