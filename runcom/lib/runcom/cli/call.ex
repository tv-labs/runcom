defmodule Runcom.CLI.Call do
  ...
  def execute(opts) do
    ...
    case execute_local(namespace, function, args, state, stdout, stderr) do
      {:ok, result} ->
  ...
  defp execute_local(namespace, function, args, state, stdout, stderr) do
    case Server.get_definition(namespace, function) do
      {:ok, %{module: module}} ->
        stdout_sink = Bash.Sink.passthrough(fn {:stdout, data} -> IO.write(stdout, data) end)
        stderr_sink = Bash.Sink.passthrough(fn {:stderr, data} -> IO.write(stderr, data) end)
        ...
        result = module.__bash_call__(function, args, nil, state_with_sinks)
        {:ok, result}
  ...
  defp handle_result({:error, message}, _stdout, stderr) when is_binary(message) do
    IO.write(stderr, message)
    {:ok, 1}
  end

  defp handle_result({:error, exit_code}, _stdout, _stderr) when is_integer(exit_code) do
    {:ok, exit_code}
  end
