defmodule Runcom.Remote.Executor do
  ...
  defp execute(module, name, args, parent) do
    stdout_sink = Bash.Sink.passthrough(fn {:stdout, data} -> send(parent, {:stdout, data}) end)
    stderr_sink = Bash.Sink.passthrough(fn {:stderr, data} -> send(parent, {:stderr, data}) end)

    state = %{stdout_sink: stdout_sink, stderr_sink: stderr_sink}

    try do
      result = module.__bash_call__(name, args, nil, state)
