...
defmodule Runcom.Remote.IntegrationTest do
  ...
  describe "direct function execution" do
    test "executes greet function with args" do
      ...
      result = module.__bash_call__("greet", ["Claude"], nil, %{})

      assert {:ok, 0, opts} = result
      assert Keyword.get(opts, :stdout) =~ "Hello, Claude!"
    end
    ...
    test "executes greet function without args uses default" do
      ...
      result = module.__bash_call__("greet", [], nil, %{})
    ...
    test "executes add function with valid args" do
      ...
      result = module.__bash_call__("add", ["5", "3"], nil, %{})

      assert {:ok, 0, opts} = result
      assert Keyword.get(opts, :stdout) =~ "8"
    end
    ...
    test "add function returns error for invalid numbers" do
      ...
      assert {:error, message} = result
      assert message =~ "invalid numbers"
    end
    ...
    test "env_check can access state env vars" do
      ...
      state = %{env: %{"HOME" => "/home/test"}}
      result = module.__bash_call__("env_check", [], nil, state)

      assert {:ok, 0, opts} = result
      assert Keyword.get(opts, :stdout) =~ "HOME=/home/test"
    end
    ...
    test "env_check handles missing env vars" do
      ...
      result = module.__bash_call__("env_check", [], nil, %{})

      assert {:ok, 0, opts} = result
      assert Keyword.get(opts, :stdout) =~ "HOME=(not set)"
    end
    ...
    test "fail function returns custom exit code" do
      ...
      result = module.__bash_call__("fail", [], nil, %{})

      assert {:ok, 42, opts} = result
      assert Keyword.get(opts, :stderr) =~ "deliberate failure"
    end
  ...
  describe "complete integration scenario" do
    ...
    test "simulates bash script sourcing and function execution" do
      ...
      state = %{env: %{"HOME" => "/home/user"}, errexit: false, pipefail: false}

      result = module.__bash_call__("greet", ["Integration"], nil, state)

      assert {:ok, 0, opts} = result
      assert Keyword.get(opts, :stdout) == "Hello, Integration!\n"
    end
    ...
    test "handles function execution with state passing" do
      ...
      # Simulate the state that Call.run would build
      state = %{
        env: %{
      ...
      assert {:ok, 0, opts} = result
      assert Keyword.get(opts, :stdout) =~ "HOME=/home/integration_user"
    end
  end
end
