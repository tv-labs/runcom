defmodule Runcom.RunbookIntegrationTest do
  ...
  describe "remote bash definition integration" do
    test "defbash function can be executed directly" do
      ...
      result = module.__bash_call__("greet", ["Integration"], nil, %{})
    ...
    test "defbash compute function works with args" do
      ...
      result = module.__bash_call__("compute", ["10", "20"], nil, %{})

      assert {:ok, 0, opts} = result
      assert Keyword.get(opts, :stdout) == "30\n"
    end
    ...
    test "defbash write_file creates files", %{tmp_dir: tmp_dir} do
      ...
      result = module.__bash_call__("write_file", [test_file, "test content"], nil, %{})

      assert {:ok, 0, opts} = result
      assert Keyword.get(opts, :stdout) =~ "wrote"
